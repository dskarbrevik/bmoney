...
def find_fuzzy_duplicates(
    ...
    for idx in range(len(df)):
        ...
        if comparison_mode == 'existing_vs_new' and len(similar) > 0:
            ...
            similar_has_existing = similar['_IS_EXISTING'].any() if '_IS_EXISTING' in similar.columns else False
            similar_has_new = (~similar['_IS_EXISTING']).any() if '_IS_EXISTING' in similar.columns else True
            
            # Only group if we're comparing existing with new (not new with new)
            if (current_is_existing and similar_has_new) or (not current_is_existing and similar_has_existing):
...
def deduplicate_transactions(
    ...
    # Find duplicates
    df_with_groups = find_fuzzy_duplicates(df, date_window, amount_tolerance, comparison_mode)
...
def merge_new_transactions(
    ...
    stats['new_duplicates_removed'] = len(new_df) - transactions_added - stats['removed_count']
