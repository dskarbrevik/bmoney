...
from budgetmoney.utils.data import update_master_transaction_df, load_master_transaction_df
...
def update(data_dir: str, 
           validate: Annotated[bool, typer.Option(help="Ensure that master transaction file has all necessary cols and features.")]=False):
    if not Path(data_dir).exists():
        raise Exception(f"The data dir: '{data_dir}' does not exist!") 
    if validate:
...
def sync(data_dir: str):
    ...
    gs_client = GSheetsClient(sheet_id=os.getenv("SPREADSHEET_ID"),
                              sa_cred_path=os.getenv("GCP_SERVICE_ACCOUNT_PATH"))

    gs_client.sync_sheet(df=df,
