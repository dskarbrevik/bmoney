...
def test_load_master_transaction_df_file_exists(mock_jsonl_data, mock_master_path):
    ...
    with patch("builtins.open", mock_open(read_data=mock_jsonl_data)), \
         patch("pandas.read_json") as mock_read_json, \
         patch("pathlib.Path.exists", return_value=True):
        
        mock_df = pd.DataFrame([{"Date": "2023-01-01", "Amount": 100, "Category": "Food"}])
        mock_read_json.return_value = mock_df

        result = load_master_transaction_df(MOCK_DATA_PATH)

        mock_read_json.assert_called_once_with(mock_master_path, orient="records", lines=True)
